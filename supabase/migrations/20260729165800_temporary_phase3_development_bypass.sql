-- Temporary Phase 3 development-preview authorization adapter.
--
-- The application records the dedicated synthetic actor as
-- staging_bypass_presenter. The authoritative tenant assignment remains
-- presenter, so this function validates the stronger temporary identity
-- contract before accepting that audit label. All other actors retain the
-- existing exact tenant-role check.
  ...
  if p_actor_role = 'staging_bypass_presenter' then
    if not exists (
      ...
      where assignment.user_id = p_actor_id
        ...
        and actor.raw_app_meta_data ->> 'access_mode' = 'staging_bypass'
        and actor.raw_app_meta_data ->> 'presenter' = 'true'
        and actor.raw_app_meta_data ->> 'bypass_expires_at' =
          '2026-08-12T23:59:59-04:00'
        and actor.raw_app_meta_data -> 'tenant_ids' ? p_tenant_id
